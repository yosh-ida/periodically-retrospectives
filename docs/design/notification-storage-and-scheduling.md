...
- 日付境界、曜日、スロット ID 生成はすべてユーザーのローカルタイムで解釈する
...
- 保存形式は `times: string[]` を維持してよいが、ユーザー入力は自由テキストに依存しない
- UI では時計 UI や候補選択 UI から時刻を選び、保存前に `HH:MM` 形式へ正規化する
- `/themes/new` の簡易通知設定編集でも、通知設定専用画面と同じ正規化ルールを使う
