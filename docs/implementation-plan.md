...
- Phase 6 完了後の追加品質向上として、通知判定とスロット生成を UTC 前提ではなくローカルタイム前提に統一する
- ローカル日付跨ぎ、曜日境界、`every-n-days` の基準日解釈、同一スロット重複防止のテストを追加する
- ローカルタイム基準の通知判定が UI、アプリ起動時判定、Service Worker 判定で一貫することを確認する

完了条件:

- 通知判定がユーザーのローカルタイム基準で一貫して動作する
- 日付境界と通知ルール解釈の再現性をテストで確認できる

### Phase 8. UI の product ready 化
...
- 通知ルール編集 UI を改善し、時刻入力を自由テキストではなく時計 UI や選択式 UI で扱えるようにする
- `every-n-days` / `weekly-days` の各ルール編集で、時刻追加・削除・曜日選択・基準日選択を誤入力しにくい操作導線に整理する
- `/themes/new` で通知設定を新規追加する際、その場で関連づけ済みの通知設定を即時編集できる導線を実装する
- 反省点作成フローで、通知設定レコードの作成だけでなく初期ルール設定まで完結できるよう UI を見直す
- UI 上の `Phase` 表示や実装途中を想起させる文言を除去し、product ready なコピーと画面構成に整える
- ダッシュボード、作成画面、詳細画面、通知設定画面の導線文言を統一し、開発用の仮案内が残っていないことを確認する

完了条件:

- 通知ルールの時刻編集をテキスト直接入力なしで完結できる
- `/themes/new` で追加した通知設定を同画面上で編集してから保存できる
- UI から Phase や実装途中を示す表示が除去され、一般ユーザー向けの画面文言に統一されている
...
3. 少なくとも次を確認する
   ...
   - 通知機能を含む Phase では、ローカルタイム基準の日付・曜日解釈と通知遷移が仕様どおりに動く
...
- `check-in` / `review` の通知判定とスロット ID 生成がユーザーのローカルタイム基準で行われる
- 通知ルール編集 UI が選択式で、誤入力しにくい product ready な操作感になっている
- `/themes/new` から通知設定の新規作成と初期編集をまとめて完了できる
- UI に開発フェーズ表記や未完成前提の文言が残っていない
- 通知から対象画面へ遷移できる
- アプリ再起動後もデータが保持される
